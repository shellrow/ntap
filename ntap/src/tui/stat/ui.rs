...
pub fn draw(f: &mut Frame, app: &mut App) {
    ...
    // Draw footer
    let footer = format!("Press <Q> to quit, <SPACE> to pause, <T> to toggle bandwidth display");
...
fn draw_top_data(f: &mut Frame, app: &mut App, area: Rect) {
        ...
        .split(area);
    {
        ...
        let rows = app
            ...
            .map(|proc| {
                let ingress_traffic: String = if app.config.display.show_bandwidth {
                    proc.traffic.formatted_ingress_bytes_per_sec.clone()
                } else {
                    proc.traffic.formatted_received_bytes.clone()
                };
                let egress_traffic: String = if app.config.display.show_bandwidth {
                    proc.traffic.formatted_egress_bytes_per_sec.clone()
                } else {
                    proc.traffic.formatted_sent_bytes.clone()
                };
                Row::new(vec![
                    proc.pid.to_string(),
                    proc.name.clone(),
